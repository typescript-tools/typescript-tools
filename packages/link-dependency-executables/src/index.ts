...
import { MonorepoRootError,  monorepoRoot as monorepoRoot_ } from '@typescript-tools/monorepo-root'
import { FindPackageError, findPackage as findPackage_ } from '@typescript-tools/find-package'
...
export type LinkDependencyExecutablesError =
    ...
    | { type: 'unable to decode dependency package.json', dependency: string, error: string }
...
const findPackage = (packagePathOrName: string) => pipe(
    ...
    TE.tryCatch(
        async () => new Promise<string>((resolve, reject) =>
            fs.readFile(
                filename,
                'utf8',
                (error, data) => error !== null && error !== undefined ? reject(error) : resolve(data)
            )),
        flow(
            E.toError,
            error => err({ type: 'unable to read file', filename, error })
        )
    )

const decodePackageJsonBin = (dependency: string) => flow(
    StringifiedJSON(PackageJsonBin).decode.bind(null),
    E.mapLeft(flow(
        errors => PathReporter.failure(errors).join('\n'),
        error => err({ type: 'unable to decode dependency package.json', dependency, error })
    )),
    TE.fromEither
)
...
const symlinkExecutable =
    (monorepoRoot: string, internalPackageDirectory: string, dependencyName: string) =>
    (executableName: ExecutableName, targetPath: Path) => {
        const symlinkLocation = path.resolve(internalPackageDirectory, 'node_modules', '.bin', executableName)
...
export const linkDependencyExecutables =
    (internalPackagePathOrName: string) =>
    (dependencyName: string): TE.TaskEither<LinkDependencyExecutablesError, void> =>
    pipe(
        TE.bindTo ('monorepoRoot') (monorepoRoot()),
        TE.bind('internalPackage', () => findPackage(internalPackagePathOrName)),
        TE.bind('dependencyManifest', ({ monorepoRoot }) => externalPackageManifest (monorepoRoot) (dependencyName)),
        TE.chain(
            ({ monorepoRoot, internalPackage, dependencyManifest }) => pipe(
                (dependencyManifest.bin ?? {}) as Record<ExecutableName, Path>,
                R.mapWithIndex(symlinkExecutable(monorepoRoot, internalPackage.location, dependencyName)),
                value => value,
